...
int uvmcopy_excludestack(pagetable_t old, pagetable_t new, uint64 stack, uint64 sz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  for (i = 0; i < sz; i += PGSIZE)
  {
    ...
    } else if (mappages(new, i, PGSIZE, (uint64)pa, flags) != 0){
      goto err;
    }
  }
  return 0;
...
int uvmcopy_onlystack(pagetable_t pagetable, uint64 stack, uint64 sz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
