...
// Create a new process, copying the parent on write.
// Sets up child kernel stack to return as if from fork() system call.
int
forkcow(void)
{
  ...
  // Copy user memory from parent to child.
  if(uvmcopyonwrite(p->pagetable, np->pagetable, p->sz) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
...
// Wake up 1 process sleeping on chan.
// Must be called without any p->lock.
void
wakeup1p(void *chan)
{
...
clone(void)
{
  ...
  // Copy user memory from parent to child.
  if(uvmcopy_excludestack(p->pagetable, np->pagetable, p->trapframe->sp, p->sz) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
...
{
  struct proc *p = myproc();
  uvmcopy_onlystack(p->pagetable, p->trapframe->sp, p->sz);
  exit(n);
}
